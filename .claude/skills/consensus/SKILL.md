...
|---|---|---|
| Claude | `claude-worker` | Claude subagent in an independent context |
...
- All external execution is read-only (codex: `-s read-only`, agy: `--mode plan`) — workers relay opinions only; the main context does the executing.
- Expect ~120s per external CLI call; a worker that exceeds it is excluded from that decision (if all are excluded, Claude decides alone).
